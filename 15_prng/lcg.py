...
def cut(data):
    ...
    plt.hist(data1, n, histtype='stepfilled', color=granat)
    plt.hist(data2, n, histtype='step', color=cegla, linewidth=3)
    plt.ylim(ymax=top + (top-bot)//2, ymin=bot-(top-bot)//2)
    plt.title("Porównanie".format(l, k, k*n, n, min(h.values()), max(h.values())))
    plt.xlabel("$X_i$")
    plt.ylabel("Liczba wystąpień")
